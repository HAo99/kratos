...
type FilterFunc func(opts *FilterOptions, in []model.Instance) []*registry.ServiceInstance
...
// WithFilter with default filter option.
...
func defaultFilter(opts *FilterOptions, ins []model.Instance) []*registry.ServiceInstance {
	items := make([]*registry.ServiceInstance, 0, len(ins))
	for _, in := range ins {
		kind := opts.Kind
...
func (r *Registry) Watch(ctx context.Context, serviceName string) (registry.Watcher, error) {
	return newWatcher(ctx, r.cli, serviceName, r.opts.group, r.opts.kind, []string{r.opts.cluster}, r.opts.filter)
}
...
func (r *Registry) GetService(_ context.Context, serviceName string) ([]*registry.ServiceInstance, error) {
	...
	fopts := &FilterOptions{Kind: r.opts.kind}
	return r.opts.filter(fopts, res), nil
}
